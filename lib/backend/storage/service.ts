...
import {
    ...
    UploadRequestInput,
} from "@/lib/backend/storage/types";
import { validateFileIntegrityHints, validateMimeType } from "@/lib/backend/storage/validation";
...
export const persistStorageMetadata = (metadata: StorageObjectMetadata): StorageObjectMetadata => {
    storageMetadata.set(metadata.objectKey, metadata);
    return metadata;
};

export const getStorageMetadata = (objectKey: string): StorageObjectMetadata | null => {
