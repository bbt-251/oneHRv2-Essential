...
import { deleteFileDocument } from "@/lib/backend/api/hr-settings/file-document-service";
...
const DeleteHeaderDialog: React.FC<DeleteHeaderDialogProps> = ({
    open,
    onOpenChange,
    document,
    onSuccess,
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try {
            const result = await deleteFileDocument("header", document.id);

            if (result) {
                showToast("Header deleted successfully", "Success", "success");
                onOpenChange(false);
                onSuccess?.();
            } else {
                showToast("Error deleting header", "Error", "error");
            }
        } catch (error) {
...
export default DeleteHeaderDialog;
