...
import { HRIssueEditModal } from "../modals/hr-issue-edit-modal";
import { IssueViewModal } from "@/components/employee/employee-engagement/modals/issue-view-modal";
...
export function HRIssueEscalationPage() {
    ...
    const handleDeleteIssue = async (issueId: string) => {
        ...
        try {
            await deleteIssue(
                issueId,
                userData?.uid ?? "",
                EMPLOYEE_ENGAGEMENT_LOG_MESSAGES.ISSUE_DELETED(
                    issues.find(i => i.id === issueId)?.issueTitle ?? "",
                ),
            );
    ...
    const handleSubmitIssue = async (issueData: Omit<IssueModel, "id">) => {
        try {
            ...
            // HR can only edit existing issues
            if (editingIssue) {
                await updateIssue(
                    { ...issueData, id: editingIssue.id },
                    userData?.uid ?? "",
                    EMPLOYEE_ENGAGEMENT_LOG_MESSAGES.ISSUE_UPDATED(issueData.issueTitle),
                );
                showToast("Issue updated successfully", "success", "success");
                setIsModalOpen(false);
            }
        } catch (error) {
            showToast("Failed to update issue", "error", "error");
    ...
    const handleStartReview = async (issue: IssueModel) => {
        setLoadingActions(prev => ({ ...prev, [`start-review-${issue.id}`]: true }));
        try {
            await updateIssue(
                { id: issue.id, issueStatus: "In Review" },
                userData?.uid ?? "",
                EMPLOYEE_ENGAGEMENT_LOG_MESSAGES.START_REVIEW(issue.issueTitle),
            );
            showToast("Review started successfully", "success", "success");
        } catch (error) {
            showToast("Failed to start review", "error", "error");
            console.error("Failed to start review:", error);
        } finally {
            setLoadingActions(prev => ({ ...prev, [`start-review-${issue.id}`]: false }));
        }
    };

    const handleEndReview = async (issue: IssueModel) => {
        setLoadingActions(prev => ({ ...prev, [`end-review-${issue.id}`]: true }));
        try {
            await updateIssue(
                { id: issue.id, issueStatus: "Reviewed" },
                userData?.uid ?? "",
                EMPLOYEE_ENGAGEMENT_LOG_MESSAGES.END_REVIEW(issue.issueTitle),
            );
            showToast("Review completed successfully", "success", "success");
        } catch (error) {
            showToast("Failed to end review", "error", "error");
            console.error("Failed to end review:", error);
        } finally {
            setLoadingActions(prev => ({ ...prev, [`end-review-${issue.id}`]: false }));
        }
    };
    ...
    const stats = {
        ...
        inReviewed: issues.filter(i => i.issueStatus === "In Review").length,
        reviewed: issues.filter(i => i.issueStatus === "Reviewed").length,
        acknowledged: issues.filter(i => i.issueStatus === "Acknowledged").length,
    };

    const uniqueTypes = Array.from(new Set(issues.map(issue => issue.issueType))).filter(Boolean);
    const uniqueStatuses = Array.from(new Set(issues.map(issue => issue.issueStatus))).filter(
        Boolean,
    );
    const uniquePriorities = Array.from(new Set(issues.map(issue => issue.priority))).filter(
        Boolean,
    );
    ...
    return (
        <div className="p-6 space-y-6 min-h-screen">
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold ">Issue Escalation Management</h1>
                    <p className="text-gray-600 mt-1">Manage and track all issue escalations</p>
                </div>
                {/* HR can only edit existing issues, not create new ones */}
            </div>
                        ...
                        <TableBody>
                            {filteredIssues.map(issue => (
                                <TableRow key={issue.id}>
                                    ...
                                    <TableCell>
                                        {issue.protectAnonymity ? (
                                            <span className="text-gray-500 italic">Anonymous</span>
                                        ) : (
                                            employees.find(emp => emp.uid === issue.createdBy)
                                                ?.firstName +
                                                " " +
                                                employees.find(emp => emp.uid === issue.createdBy)
                                                    ?.surname || "Unknown"
                                        ...
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {issue.issueStatus === "Open" && (
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => handleStartReview(issue)}
                                                    disabled={
                                                        loadingActions[`start-review-${issue.id}`]
                                                    }
                                            ...
                                            {issue.issueStatus === "In Review" && (
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => handleEndReview(issue)}
                                                    disabled={
                                                        loadingActions[`end-review-${issue.id}`]
                                                    }
