...
interface OrderItemEditModalProps {
    ...
    editingItem?: OrderItemModel | null;
}
...
export function OrderItemEditModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast("Order item updated successfully", "Success", "success");
        } catch (error) {
            console.error("Error updating order item:", error);
            showToast("Failed to update order item", "Error", "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto z-[100]">
                <DialogHeader>
                    <DialogTitle className="text-2xl font-semibold">Edit Order Item</DialogTitle>
                    <DialogDescription>Update the order item details below</DialogDescription>
                </DialogHeader>
