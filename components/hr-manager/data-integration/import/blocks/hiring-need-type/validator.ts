...
/**
 * Hiring Need Type validation logic
 * This handles validation rules specific to hiring need type import data
 */

/**
 * Validates CSV data for hiring need type import
 ...
 */
export async function validateHiringNeedTypeData(
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        // Step 4: Hiring Need Type-specific validation
        const hiringNeedTypeErrors = validateHiringNeedTypeSpecificFields(row);
        if (Object.keys(hiringNeedTypeErrors).length > 0) {
            Object.assign(fieldErrors, hiringNeedTypeErrors);
            hasErrors = true;
        }
...
/**
 * Validates hiring need type-specific reference fields
 */
function validateHiringNeedTypeSpecificFields(row: Record<string, any>): Record<string, string> {
