...
/**
 * Level Of Education validation logic
 * This handles validation rules specific to level of education import data
 */

/**
 * Validates CSV data for level of education import
 ...
 */
export async function validateLevelOfEducationData(
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        // Step 4: Level Of Education-specific validation
        const levelOfEducationErrors = validateLevelOfEducationSpecificFields(row);
        if (Object.keys(levelOfEducationErrors).length > 0) {
            Object.assign(fieldErrors, levelOfEducationErrors);
            hasErrors = true;
        }
...
/**
 * Validates level of education-specific reference fields
 */
function validateLevelOfEducationSpecificFields(row: Record<string, any>): Record<string, string> {
