...
/**
 * Training Length validation logic
 * This handles validation rules specific to training length import data
 */

/**
 * Validates CSV data for training length import
 ...
 */
export async function validateTrainingLengthData(
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        // Step 4: Training Length-specific validation
        const trainingLengthErrors = validateTrainingLengthSpecificFields(row);
        if (Object.keys(trainingLengthErrors).length > 0) {
            Object.assign(fieldErrors, trainingLengthErrors);
            hasErrors = true;
        }
...
/**
 * Validates training length-specific reference fields
 */
function validateTrainingLengthSpecificFields(row: Record<string, any>): Record<string, string> {
