...
/**
 * Training Complexity validation logic
 * This handles validation rules specific to training complexity import data
 */

/**
 * Validates CSV data for training complexity import
 ...
 */
export async function validateTrainingComplexityData(
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        // Step 4: Training Complexity-specific validation
        const trainingComplexityErrors = validateTrainingComplexitySpecificFields(row);
        if (Object.keys(trainingComplexityErrors).length > 0) {
            Object.assign(fieldErrors, trainingComplexityErrors);
            hasErrors = true;
        }
...
/**
 * Validates training complexity-specific reference fields
 */
function validateTrainingComplexitySpecificFields(
