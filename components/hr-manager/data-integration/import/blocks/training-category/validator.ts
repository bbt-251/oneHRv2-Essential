...
/**
 * Training Category validation logic
 * This handles validation rules specific to training category import data
 */

/**
 * Validates CSV data for training category import
 ...
 */
export async function validateTrainingCategoryData(
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        // Step 4: Training Category-specific validation
        const trainingCategoryErrors = validateTrainingCategorySpecificFields(row);
        if (Object.keys(trainingCategoryErrors).length > 0) {
            Object.assign(fieldErrors, trainingCategoryErrors);
            hasErrors = true;
        }
...
/**
 * Validates training category-specific reference fields
 */
function validateTrainingCategorySpecificFields(row: Record<string, any>): Record<string, string> {
