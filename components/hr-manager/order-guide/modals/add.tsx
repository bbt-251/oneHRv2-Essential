...
interface OrderGuideFormModalProps {
    isOpen: boolean;
    onClose: () => void;
}

export function OrderGuideFormModal({ isOpen, onClose }: OrderGuideFormModalProps) {
    const { employees, orderItems, trainingMaterials, trainingPaths, hrSettings } = useFirestore();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.orderGuideName.trim()) return;

        const guide: Omit<OrderGuideModel, "id"> = {
            timestamp: getTimestamp(),
            orderGuideID: `OG-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`,
        ...
        // Save the guide using the service
        try {
            await OrderGuideService.createOrderGuide(
                guide,
                userData?.uid ?? "",
                ORDER_GUIDE_LOG_MESSAGES.ORDER_GUIDE_CREATED(guide.orderGuideName),
            );
            onClose();
            ...
            showToast("Order guide created successfully", "Success", "success");
        } catch (error) {
            console.error("Error saving order guide:", error);
            showToast("Failed to create order guide", "Error", "error");
        } finally {
            setIsLoading(false);
        }
    };

    const handleAddEmployee = (uid: string) => {
    ...
    const handleRemoveEmployee = (uid: string) => {
        setFormData(prev => ({
            ...prev,
            associatedEmployees: prev.associatedEmployees.filter(emp => emp.uid !== uid),
        }));
    };
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto z-[100]">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 mt-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <Label htmlFor="orderGuideName" className="text-sm font-medium">
                                        Order Guide Name <span className="text-danger-500">*</span>
                                    </Label>
                                    <Input
                                        id="orderGuideName"
                                ...
                                {formData.associatedEmployees.length > 0 && (
                                    <div className="space-y-2 mt-3">
                                        {formData.associatedEmployees.map(employee => (
                                            <div
                                                key={employee.uid}
                                                className="flex items-center justify-between p-3 rounded-lg border border-accent-200"
                                            >
                    ...
                    {/* Form Actions */}
                    <div className="flex justify-end gap-3 pt-6 border-t border-gray-200 z-[100]">
                        ...
                        >
                            {isLoading ? "Creating..." : "Create Order Guide"}
                        </Button>
                    </div>
                </form>
            </DialogContent>
